...
/// ViewModel for TextView component with two-way variable binding
class CTextViewViewModel: CBaseViewModel<DSL.Model.Component.TextView.C> {
    ...
    private func setupTwoWayBinding() {
        guard let bindVar = currentProperties.bindVariable else {
            return
        }
        ...
        textChanged
            ...
            .sink { [weak self] newText in
                ...
                do {
                    ...
                    service.context.logger.error("Failed to update variable '\(bindVar)': \(error)")
                }
            }
    ...
    func isCharacterAllowed(_ string: String, in range: NSRange, currentText: String) -> Bool {
        if let maxLength = currentProperties.maxLength {
